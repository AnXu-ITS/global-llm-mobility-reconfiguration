...
const work=path.dirname(here),root=path.dirname(work),out=path.join(work,'figures_v2');
...
function legend(s,x=200,y=105){M.forEach((m,i)=>{shape(s,'rect',x+i*252,y+7,20,16,C[m]);txt(s,{B0:'B0 Ground',B1:'B1 Rule',B2:'B2 Heuristic',B4b:'B4b LLM'}[m],x+28+i*252,y,207,30,21);});}
...
for(const key of ['D1','H1','B1']){if(!f[key])continue;const [xx,yy]=tr([f[key].x,f[key].y]);shape(s,key==='B1'?'diamond':'ellipse',xx-4,yy-4,8,8,key==='B1'?C.disruption:C.ink);const offs={D1:[-57,-24],H1:[8,3],B1:[9,-24]}[key];txt(s,key==='B1'?'Closure':key,xx+offs[0],yy+offs[1],68,23,17,true);}
for(const key of ['V3','D2','H2','C2','C3']){if(!f[key])continue;const [xx,yy]=tr([f[key].x,f[key].y]);shape(s,'rect',xx-4,yy-4,8,8,'#FFFFFF',key==='V3'?C.air:'#8C7168',1.6);txt(s,key,xx+7,yy-12,36,24,16,true,key==='V3'?C.air:'#735C55');}
...
[['Primary ground',C.ground],['Detour',C.ground],['Primary air',C.air],['Auxiliary ground','#7D95A8'],['Other air','#BF9A6A']].forEach(([l,c],i)=>{if(l==='Detour'){line(s,45+i*244,652,56+i*244,652,c,2.5);line(s,62+i*244,652,73+i*244,652,c,2.5);}else line(s,45+i*244,652,73+i*244,652,c,2.5);txt(s,l,81+i*244,636,205,30,18);});
...
line(s,fx(300),250,fx(360),250,C.air,7);shape(s,'ellipse',fx(300)-6,244,12,12,C.air);shape(s,'diamond',fx(360)-8,242,16,16,C.disruption);
txt(s,'300 s\nInitial dispatch selected',fx(300),131,276,68,23,true);
txt(s,'360 s\nFailure and replacement decision',fx(360)-20,293,390,72,23,true,C.disruption);
line(s,fx(360),443,fx(420),443,C.ground,7);line(s,fx(420),443,fx(602),443,C.ground,7);
[360,420,602].forEach(t=>shape(s,'ellipse',fx(t)-6,437,12,12,t===360?C.ground:C.ground));
txt(s,'Pending',fx(302),257,170,30,21,false,C.air);txt(s,'Pending',fx(362),453,170,30,21,false,C.ground);
txt(s,'420 s\nGround executes',fx(420)-15,482,220,65,23,true,C.ground);
txt(s,'602 s\nMission completes',fx(602)-182,482,217,65,23,true,C.ground,'right');
txt(s,'Ground travel',fx(500),389,235,35,23,false,C.ground);
line(s,fx(480),126,fx(480),578,C.disruption,1.8);txt(s,'480 s\nDeadline',fx(480)+10,136,173,64,23,true,C.disruption);
...
const candidate=path.join(here,'figures_v2_candidate.pptx');
...
const final=path.join(out,process.env.FIGURE_PPTX_NAME??'MANUSCRIPT_FIGURES_2_to_8_v2.pptx');
