...
for path in OUT.glob('*.pdf'):
    ...
    assert size>=6.49 and not p.get_images(),path.name
    letters=['a','b'] if path.stem=='Figure4' else ['a','b','c']
    labels=[next(s for s in spans if s['text']==letter and abs(s['size']-9)<.01) for letter in letters]
