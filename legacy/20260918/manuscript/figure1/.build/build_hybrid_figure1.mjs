...
const MOD='C:/Users/user/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules';
...
const SK='C:/Users/user/.codex/plugins/cache/openai-primary-runtime/presentations/26.904.11930/skills/presentations';
...
const W=1840,H=1380,name=process.env.HYBRID_STEM??'Figure1_vC_hybrid_reference';
...
text('Figure 1. Overview of the Ground-Low-Altitude Mobility Manager',56,5,1728,60,40,true,C.ink,'center');
...
shape('rect',32,1006,271,51,'#FFFFFF');line(47,1041,203,1041,C.ink,3);[47,125,203].forEach(x=>line(x,1031,x,1044,C.ink,3));text('0',39,1008,31,27,24);text('2',117,1008,31,27,24);text('4 km',194,1008,65,27,24);text('Schematic scale',282,1027,176,28,21,false,'#5D646A','right');
...
for(let i=0;i<mods.length;i++){
 ...
 await img(id,549,y-20,159,i===3?120:144);text(label,720,y+13,157,77,29,i===3);
 if(i<mods.length-1)arrow(733,y+(i===3?193:111),733,mods[i+1][2]-10,C.blue,3);
}
shape('rect',527,1235,358,94,'#F5F7FC',C.border,1.7);await img('simulators',537,1244,116,75);text('SUMO + BlueSky',661,1250,218,57,27);arrow(733,1184,733,1225,C.blue,3);
line(529,407,529,797,C.purple,2.4,true);arrow(529,407,557,407,C.purple,2.4,true);arrow(529,797,557,797,C.purple,2.4,true);
shape('rect',496,583,78,82,'#FFFFFF');text('Shared\ndata',497,589,77,68,24,false,C.purple,'center');
line(902,283,902,1117,C.air,2.3,true);arrow(902,283,885,283,C.air,2.3,true);arrow(885,1117,902,1117,C.air,2.3,true);shape('rect',801,1002,109,32,'#FFFFFF');text('Feedback',805,1002,103,32,22,false,'#AE6C32','center');
...
text('Control input',751,1188,146,32,22,false,C.green,'right');
...
const scenario=[['E1','Air support\ndisruption','e1',951,233,C.blue],['E2','Air-layer\nreconfiguration','e2',1194,233,C.blue],['E3','Compound\ndisturbance','e3',951,555,C.green],['E4','Observation\nsensitivity','e4',1194,555,C.air]];
for(const [badge,label,id,x,y,c] of scenario){shape('rect',x,y,58,42,c,'#41536B',1.3);text(badge,x,y,58,42,30,false,'#FFFFFF','center');text(label,x+70,y-3,160,66,25);await img(id,x-4,y+76,222,218);}
...
const curve=(f,n=65)=>Array.from({length:n},(_,k)=>{let q=k/(n-1);return[1495+q*290,f(q)]});
route(curve(q=>650-45*Math.sin(q*Math.PI)-18*Math.sin(q*Math.PI*2)),C.ground,5);
route(curve(q=>650+27*Math.sin(q*Math.PI*2)), '#898B94',4,true);
dot(1495,650,10,C.ground,'#BCD5EC',3);dot(1785,650,10,C.red,'#F3C0C2',3);dot(1605,604,5,C.ground,C.ground,1);line(1605,610,1605,685,C.ground,1.8,true);
text('Start',1467,666,65,31,25);text('End',1753,666,57,31,25);text('Reconfigure',1540,563,152,31,23,false,C.ink,'center');
line(1496,724,1564,724,C.ground,5);text('Timely recovery',1580,706,215,36,26);line(1496,762,1564,762,'#898B94',4,true);text('Delayed recovery',1580,744,220,36,26);
